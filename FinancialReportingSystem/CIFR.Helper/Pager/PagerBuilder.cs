...
namespace CIFR.Helper.Pager
{
    ...
    internal class PagerBuilder
    {
        ...
        internal MvcHtmlString RenderPager()
        {
            ...
            //{
            //    //return
            //    //    MvcHtmlString.Create(string.Format("<div style=\"color:red;font-weight:bold\">{1}</div>{0}",
            //    //                          _pagerOptions.PageIndexOutOfRangeErrorMessage));
            //}
            if (_pagerOptions.AutoHide && _totalPageCount <= 1)
                return MvcHtmlString.Empty;

            if (_pageIndex > _totalPageCount) _pageIndex = _totalPageCount;
        ...
        private string GenerateAnchor(PagerItem item)
        {
            ...
            if (_pagerOptions.UseJqueryAjax)
            {
                ...
                if (_ajaxOptions != null && (!string.IsNullOrEmpty(_ajaxOptions.OnFailure) || !string.IsNullOrEmpty(_ajaxOptions.OnBegin) || (!string.IsNullOrEmpty(_ajaxOptions.OnComplete) && _ajaxOptions.HttpMethod.ToUpper() != "GET")))
                {
        ...
        private RouteValueDictionary GetCurrentRouteValues(ViewContext viewContext)
        {
            ...
            {
                var invalidParams = new[] { "x-requested-with", "xmlhttprequest", "returnurl", _pagerOptions.PageIndexParameterName.ToLower() };
